...
void Amplifier::set_current(double current) {
    ...
    command_channel_.set_value(current_limiter_.limit(current_command_) / command_gain_);
}
...
double Amplifier::get_current_sense() const {
    ...
    std::cout << "WARNING: Amplifier <" << name_ << "> current sense channel. Returning 0." << std::endl;
    return 0.0;
}
