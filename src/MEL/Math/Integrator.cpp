...
Integrator::Integrator(double initial_condition, Integrator::Technique technique) :
...
double Integrator::integrate(double x, Time t) {
    switch (technique_) {
        case Technique::Trapezoidal:
            if (step_count_ > 0)
                integral_ += (t.as_seconds() -
                    last_t_.as_seconds()) * (0.5 * (last_x_ + x));
            break;
        case Technique::Simpsons:
            if (step_count_ > 1)
                integral_ += 0.166666666666666 * (t.as_seconds() -
                    last_t_.as_seconds()) * (last_last_x_ + 4 * last_x_ + x);
