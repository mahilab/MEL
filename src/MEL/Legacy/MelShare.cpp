#include <MEL/Legacy/MelShare.hpp>
...
{
    int* size;
    Mutex mutex(mutex_name, Mutex::Open);
    switch (mutex.try_lock()) {
        case  Mutex::NotOpen:
        return -2;
        case  Mutex::LockAbandoned:
        return -3;
        case  Mutex::LockTimeout:
        return -4;
        case  Mutex::LockFailed:
        return -5;
        case Mutex::LockAquired:
        size = static_cast<int*>(region_size.get_address());
        switch (mutex.release()) {
            case Mutex::ReleaseFailed:
            return -6;
        }
        return *size;
    }
...
mutex_(mutex_name_, Mutex::Create)
{
    try {
