    ...
    sanity_check_on_enable_(sanity_check_on_enable)
{
    ...
    std::strcpy(options_str_, options_.build().c_str());
}
...
void Q8Usb::enable() {
    if (!enabled_) {
        ...
        result = hil_set_card_specific_options(q8_usb_, options_str_, std::strlen(options_str_)); // TODO: make this optional or configured in another way
        if (result < 0) {
            std::cout << "Failed" << std::endl;
            print_quarc_error_message(result);
            return;
...
void Q8Usb::factory_reset() {
    ...
    std::strcpy(factory_options_str, factory_options.build().c_str());
    t_error result = hil_set_card_specific_options(q8_usb_, factory_options_str, std::strlen(factory_options_str));
    if (result < 0) {
...
void Q8Usb::zero_encoders() {
    if (enabled_ && encoder_channels_count_ > 0) {
        ...
        std::vector<int32> enc_zero_counts(encoder_channels_count_, 0);
...
void Q8Usb::offset_encoders(std::vector<int32> offset_counts) {
    if (enabled_ && encoder_channels_count_ > 0) {
...
void Q8Usb::set_encoder_quadrature_factors(std::vector<uint32> quadrature_factors) {
...
channel_vec Q8Usb::get_q8_encrate_channels(channel_vec enc_channels) {
    std::vector<uint32> vel_channels = enc_channels;
