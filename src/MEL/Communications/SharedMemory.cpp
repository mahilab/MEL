...
MapHandle SharedMemory::create_or_open(const std::string& name, std::size_t size) {
    MapHandle map;
    ...
    if (map == -1)
        LOG(Error) << "Could not create file mapping object " << name << " (Error # " << errno << " - " << strerror(errno) << ")";
    else {
        int r = ftruncate(map, size);
        if (r != 0) {
            LOG(Error) << "Could not truncate file mapping object " << name << " (Error # " << errno << " - " << strerror(errno) << ")";
        }
