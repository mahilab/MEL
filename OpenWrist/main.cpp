...
int main(int argc, char * argv[]) {
	...
	/* instatiate and initialize Q8 USB */
	...
	while (stop == 0) {
		...
		std::cout << ow.joint_torques_[0] << std::endl;

		ow.command_joint_torques();
		/* END CONTROLLER SPECIFIC CODE */
		/* log desired data */
		data_log << (float)sample_number / frequency << "\t" << (double)time_elapsed_loop.count() / 1000000000.0 << "\t" << ow.joint_positions_[1] << "\t" << ow.joint_velocities_[1]
			<< "\t" << ow.joint_torques_[1] << "\t" << ow.motor_torques_[1] << "\t" << ow.motor_currents_[1] << "\t" << q8->ao_voltages_[1] << "\t" << q8->ai_voltages_[1] << std::endl;
