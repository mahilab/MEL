...
main_window.setWindowTitle('MEL Scope v' + ver + ' (untitled)')
...
def prompt_connect_melshare():
    ...
    input, ok = QtGui.QInputDialog.getText(
        widg, 'MELShare', 'Enter the MELShare Name:')
    if ok:
...
class ScopeModule(QtGui.QWidget):
    ...
    def __init__(self):
        ...
        # the number of data points in each scope at any given time
        self.samples = self.duration * fps_target
    ...
    def clear(self):
        ...
        for i in range(melshare_size):
            self.all_curves.append(self.plot.plot(
                pen=pyqtgraph.mkPen(curve_colors[i], width=curve_size)))
...
def configure_scope_modules(titles):
    for (scope, title) in zip(scope_modules, titles):
        scope.configure(title)
...
def prompt_resize_grid():
    ...
    selection, ok = QtGui.QInputDialog.getItem(
        widg, 'Grid Size', 'Select Grid Size:', options_display, 0, False)
    if ok:
...
def resize_grid():
    ...
    if new_count != scope_count:
        ...
        # resize window
        main_window.setFixedSize(
            400 * grid_cols * resolution_scale, 300 * grid_rows * resolution_scale)
        status_bar.showMessage('')
...
def open_scope():
    ...
    if filepath:
        ...
        main_window.setWindowTitle('MEL Scope v' + ver + ' (' + filename + ')')
...
def deploy_config():
    global config, melshare_name, grid_rows, grid_cols
    ...
    resize_grid()
    connect_melshare()
    configure_scope_modules(titles)
