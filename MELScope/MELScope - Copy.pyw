...
def add_melshare(melshare_name, melshare_mode):
    if melshare_name not in MELSHARE_NAMES:
        MELSHARE_NAMES.append(melshare_name)
        MELSHARE_MODES[melshare_name] = melshare_mode
        MELSHARE_SIZES [melshare_name] = None
        MELSHARE_CONNECTIONS[melshare_name] = False
        MELSHARE_DOUBLE_ARRAYS[melshare_name] = None
        MELSHARE_BUFFERS[melshare_name] = None
        print_melshare_info()

def remove_melshare(melshare_name):
    if melshare_name in MELSHARE_NAMES:
        MELSHARE_NAMES.remove(melshare_name)
        del MELSHARE_MODES[melshare_name]
        del MELSHARE_SIZES[melshare_name]
        del MELSHARE_CONNECTIONS[melshare_name]
        del MELSHARE_DOUBLE_ARRAYS[melshare_name]
        del MELSHARE_BUFFERS[melshare_name]
        print_melshare_info()

def connect_melshare(melshare_name):
    if melshare_name in MELSHARE_NAMES:
        result = MELSHARE_DLL.get_map_size(melshare_name)
        if result >= 0:
            MELSHARE_SIZES[melshare_name] = result
            MELSHARE_CONNECTIONS[melshare_name] = True
            MELSHARE_DOUBLE_ARRAYS[melshare_name] = ctypes.c_double * result
            MELSHARE_BUFFERS[melshare_name] = MELSHARE_DOUBLE_ARRAYS[melshare_name]
        else:
            MELSHARE_SIZES[melshare_name] = None
            MELSHARE_CONNECTIONS[melshare_name] = False
            MELSHARE_DOUBLE_ARRAYS[melshare_name] = None
...
def sample_data():
    ...
    result = MELSHARE_DLL.read_double_map(melshare_name, ctypes.byref(
...
def reset_samples():
    ...
    MELSHARE_DLL.read_double_map(melshare_name, ctypes.byref(
        melshare_read_buffer), melshare_size)
...
def validate_curve_properties():
    ...
    if melshare_size > len(curve_colors):
        for i in range(len(curve_colors), melshare_size):
            curve_colors.append(default_curve_colors[
...
class ScopeModule(QtGui.QTabWidget):  # or QtGui.QGroupBox or QTabWidget
    ...
    def refresh(self):
        ...
        for i in range(melshare_size):
            if self.filter[i]:
                # update curve
                new_pen = pyqtgraph.mkPen(curve_colors[i], style=curve_line_options[
                                          curve_lines[i]], width=curve_sizes[i])
                new_curve = pyqtgraph.PlotCurveItem(
                    pen=new_pen, name=curve_names[i])
    ...
    def rename_data(self, index, event=None):
        ...
        self.temp_line_edit1.editingFinished.connect(
            lambda: self.confirm_data_rename(index))
...
grid_options = [(r + 1, c + 1) for r in range(max_grid_rows)
...
class ConfigureDataDialog(QtGui.QDialog):
    def __init__(self, parent=None):
        ...
        for i in range(melshare_size):
            ...
            new_color_button.clicked.connect(
                self.prompt_color_dialog_factory(i))
            ...
            new_line_combo_box.setCurrentIndex(
                curve_line_options.keys().index(curve_lines[i]))
    ...
    def prompt_color_dialog(self, index):
        current_color = self.color_buttons[
...
connect_action = QtGui.QAction('Connect &MELShare...', main_window,
                               shortcut='Ctrl+M', statusTip='Connect to a MELShare map', triggered=prompt_connect_melshares)
...
def main_loop():
    ...
    if melshare_connected:
        if sample_data():
            ...
            melshare_connected = False
            status_bar.showMessage(
                'Lost connection to MELShare <' + melshare_name + '>')
